...
const navItems = [
  ...
  {icon: 'receipt-long', label: 'Prescriptions'},
...
function HomeScreen(): React.JSX.Element {
  ...
  const colors = {
    ...
    navInactive: isDarkMode ? 'rgba(248,249,250,0.6)' : 'rgba(47,79,79,0.6)',
  };
              ...
              {navItems.map(item => (
                <Pressable key={item.label} style={styles.navItem}>
                  <MaterialIcons
                    name={item.icon}
                    size={24}
                    color={item.active ? palette.primary : colors.navInactive}
                  />
                  <Text
                    style={[
                      styles.navLabel,
